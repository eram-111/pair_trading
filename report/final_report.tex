...
\title{\vspace{-1.6em}\textbf{Stocks of a Sector Rise Together:\\
Pairs Trading With a Learned Betting Rule}}

\author{Alex Bachynsky \quad Golam Eram \quad Jaskaran Narula \\
\normalsize CSC311: Summer 2026 Final Project (Option 2)\\
\normalsize Code: https//github.com/eram-111/pair_trading}
\date{}
...
\begin{abstract}
\noindent
Some stocks normally move together. However, when they drift apart, "Staistical Arbitrage" bets
...
only when confident. We explored 8 different approaches to this problem: 2 ways of choosing pairs $\times$ 4
ways of deciding to trade, and tested all of them identically on 10 years of
real market history. Our models reflect real learning, on data they had never seen they
rank opportunities better than guessing. However, none make money after fees, because
...
"Statistical arbitrage," or "Pairs trading," is a strategy that bets on the latter. When a gap
opens, you buy the stock that fell behind and bet against the one that ran ahead.
If they come back together, you win on both sides. If they dont, you lose.
\cite{gatev2006}.

The problem is that most gaps do not close; in our data only about 1
in 4 does. The question, therefore, that our report seeks to answer is: \textbf{``when a gap opens, is this one
worth trading?''} That is a prediction problem, which is what machine learning is
for. In our analysis we explored 8 different approaches to this problem in two ways, ran all 8 through identical
code, and looked at the test data exactly once. Our submitted code regenerates
every number in this report from the raw prices with one command.
...
\midrule
E0 & Trades every gap. No learning at all, this is our baseline. \\
...
Gatev et al. \cite{gatev2006} wrote the paper that made this strategy
famous: pick pairs whose prices had historically tracked each other, trade when
they drift apart, and close when they re-converge. It made was able to reliably produce significant gains on
US market history from 1962--2002. Later researchers found those profits shrinking as the
...
stock (our Track~A). Zhang et al. \cite{zhang2014} instead group
companies by company features, such as profitability, or growth,
valuation, rather than merely price history (our Track~B); we also borrow their
...
However, what our report aims to measure whether machine-learning provides a significant advantage, by
...
trading results come out at the end. The grey boxes are shared by all 8 versions, same code, same settings, same random seed, and only the white boxes change. So if one version beats another, it is because of the thing we changed,
...
2, the gap is unusually large and we call that a trigger. We wait one
...
Every trigger becomes one training example. The answer to predict is:
did the gap shrink by half within 5 days? (Yes about 24\% of the time.) The
clues are 7 numbers: how big the gap is, how jumpy the pair usually is,
...
every period, the number our model must beat. We looked at the test years
exactly once, at the very end, after everything was locked.
...
\textbf{Track B: pairs that look alike.} Following Zhang et al.
...
businesses. Price history plays no part. This gives 39 regroupings and 242
...
training examples, anything more flexible risks memorising. Across tracks,
...
makes its answer a straight line again, so it should behave like E1 (Section~6.4). Across tracks it tracks E1 almost exactly on both.

\textbf{E3} is a small neural network: 7 inputs, one hidden layer of 8 or 16 units, one
output, trained with early stopping. It is the only model that can bend its
boundary. Across tracks it inverts E1's pattern, worst on Track~A, best
...
whether a model knew anything, Table~\ref{tab:money} whether that
knowledge was worth anything, and they get different answers, which is
the heart of this report.

Starting with the returns, after fees, all 8 versions lost. The best, Track~A with
...
trade. The fee is $40$ bps. This is the somewhat disappointing result of our project in sum: \textbf{the strategy earns about 39 cents per \$100 traded, and costs 40
...
21\% of the time, and our models are correct 62--73\%.

That looks like a huge improvement, but in actuality It is not. The last row of Table~\ref{tab:predict} shows a strategy that
...
How much of the improvement is skill? To find out, we built 1{,}000 fake strategies per model. Each trades exactly as
...
E1 and E2 come from different angles, one learns the boundary directly,
...
fees, and both ranked 88--89 against their random controls.
...
strategies apart, the differences we were admiring were noise.
...
volatile,  which is when our triggers fire, so our results are
...
few hundred. Our submitted code reproduces all of it.

\bigskip
\noindent\textbf{Contributions.} Alex Bachynsky: literature review, report writing,
results analysis. Jaskaran Narula: data pipeline, features, Track~B fundamentals, E1.
Golam Eram: pair-forming pipeline, trade simulator, evaluation code (metrics,
